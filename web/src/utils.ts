...
const MAX_BUFFERED_AUDIO_SEC = 2;
const RESYNC_LOG_INTERVAL_SEC = 5;
...
export function scheduleChunk(
  ...
  const queuedAheadSec = Math.max(0, nextStartTime - now);

  if (queuedAheadSec > MAX_BUFFERED_AUDIO_SEC) {
  ...
  if (
    nextStartTime < now &&
    audioCtx.currentTime - lastResyncLogAt > RESYNC_LOG_INTERVAL_SEC
  ) {
    lastResyncLogAt = audioCtx.currentTime;
    console.log(
